...
## Статус
- [x] Этап 0 — каркас, PWA, деплой на GitHub Pages
- [x] Этап 1 — колоды (иерархия), 6 встроенных типов заметок, шаблоны, пропуски, добавление и редактирование
- [x] Этап 2 — планировщик SM-2, экран учёбы, отмена, флажки, откладывание, приостановка, аудио и TTS, ввод ответа
- [x] Базовый поиск по карточкам, настройки колоды, светлая и тёмная темы
- [x] Этап 3 — импорт CSV / TXT (автоопределение разделителя и кодировки, заголовки Anki, сопоставление колонок, дубли) и APKG (заметки, карточки, прогресс, история, медиа, настройки колод)
- [x] Этап 4 — резервная копия и восстановление (.zip), экспорт колоды в TXT для Anki и CSV, напоминание о копии
- [x] Облачная синхронизация через Supabase: вход по email и паролю, отправка только изменений, картинки в хранилище (schema: supabase/schema.sql)
- [ ] Экспорт в APKG (нужна проверка на настоящем Anki)
- [ ] Этап 5 — расширенный поиск (`deck:`, `tag:`, `is:`), массовые действия
- [ ] Этап 6 — статистика, управление типами заметок
